/* MenuInterpreter
  Version 1.7
  (c) 2009-2010, 2012, 2014, 2016 by Malte Marwedel
...
unsigned short menu_gfxindexstate[MENU_MULTIGFX_MAX];
#endif
...
void menu_text_draw_base(SCREENPOS x, SCREENPOS y, unsigned char font,
	...
	while (index < 10000) {
		...
		SCREENPOS ox = x;
		unsigned char width = menu_char_draw(x, y, font, cdraw);
...
void menu_label(void) {
	...
	MENU_DEBUGMSG("Drawing label %i, %i with font %i, storage %i, addr %i\n", px, py, fonts, storage, textaddr);
...
static void menu_basicbutton(unsigned char hasfocus, unsigned char offx, unsigned char offy) {
	...
	MENUADDR textaddr  = menu_assemble_addr();
	unsigned char fonts = menu_byte_get_next();
	MENU_DEBUGMSG("Drawing button %i, %i with font %i, storage %i, addr %i\n", px, py, fonts, storage, textaddr);
	if (hasfocus) {
		fonts = fonts >>4;
	} else
		fonts = fonts & 0x0f;
...
void menu_gfx(unsigned char hasfocus) {
	...
	if ((options >> MENU_OPTIONS_MULTIGFX) & 1) {
		short images = menu_byte_get(gfxaddr) + (menu_byte_get(gfxaddr+1) << 8);
		unsigned char arrindex = menu_byte_get(gfxaddr+2);
		short index = menu_gfxindexstate[arrindex];
...
#endif
	MENU_DEBUGMSG("Drawing gfx at %i, %i, compressed: %i, storage: %i, address: %i\n", px, py, compressed, storage, gfxaddr);
	for (y = py; y < py+sy; y++) {
		for (x = px; x < px+sx; x++) {
