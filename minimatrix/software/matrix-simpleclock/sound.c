...
/*Keeping the pin as output at the end of the beep avoids clicking noise, which
happend when only pullups were used */
...
ISR(TCC0_OVF_vect) {
	...
	if (soundcycles == 0) {
		...
		TCC0.CCA--; //decrease volume at end of beep
	} else if (TCC0.CCA < g_soundton) {
		TCC0.CCA++; //increase volume
	}
}
...
void sound_beep(uint8_t volume, uint16_t frequency, uint16_t duration) {
	...
	if (g_soundcycles == 0) {
		PORTC.OUTCLR = 1;
		PORTC.DIRSET = 1;
		PORTC.PIN0CTRL = PORT_INVEN_bm; //the transistors invert again. So 0 results in low level.
	...
	if ((F_CPU)/frequency > 100) {
		g_soundton = (F_CPU/frequency)/(4*(256-volume)); //50% max volume limit
